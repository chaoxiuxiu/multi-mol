...
class MTDataModule(LightningDataModule):
    def __init__(self, _config, dist=True):
        ...
        self.dm_keys = datamodule_keys  # [ "pretrain"]保存数据模块键列表。
        # self.dm_dicts = {
        #     "pretrain": pretrainDataModule,
        # }
        self.dm_dicts = {key: _datamodules[key](_config) for key in datamodule_keys}#根据键列表创建数据模块字典。_datamodules 是一个字典，包含了数据模块类，使用 _config 初始化这些数据模块
        # dms = [pretrainDataModule]

        self.dms = [v for k, v in self.dm_dicts.items()]
        #print(self.dms)#[<datamodules.pre_datamodule.pretrainDataModule object at 0x7f83fcb6ec40>]
        #[<datamodules.MoleculeNetClassify_datamodule.MoleculeNetClassifyDataModule object at 0x7f46e1f2fbb0>]
    ...
    def setup(self, stage):
        ...
        #print(f"stage:{stage}")stage:fit
        #print(f'self.dms:{self.dms}')self.dms:[<datamodules.pre_datamodule.pretrainDataModule object at 0x7f73af797af0>]
        ...
        #print(f'self.train_dataset:{self.train_dataset}')self.train_dataset:<torch.utils.data.dataset.ConcatDataset object at 0x7f71e0134040>
        # 分布式划分
        if self.imbsampler:
            pass
        elif self.dist:
