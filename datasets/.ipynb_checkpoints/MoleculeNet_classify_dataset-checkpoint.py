...
class MoleculeNet_classify_dataset(BaseDataset):
    ...
    def __getitem__(self, index):
        #######  set image-smiles as input ########
        return self.get_smiles(index),self.get_image(index),self.get_label(index)

        # ######  set only image as input ########
        # smiles = ""
        # return smiles,self.get_image(index),self.get_label(index)

        ######  set only smiles as input ########
        # images = torch.ones(3,224,224) * 255
        # return self.get_smiles(index),images,self.get_label(index)