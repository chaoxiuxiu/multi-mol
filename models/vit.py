...
class Attention(nn.Module):
    ...
    def forward(self, x, register_hook=False):
        ...
        # (B,num_heads,N,nums_per_head) @ (B,num_heads,nums_per_head,N) -> (B,num_heads,N,N)     并在每个元素上乘以self.scale
...
class Block(nn.Module):# Vision Transformer 模型中的块，包括自注意力层和 MLP 层
    ...
    def forward(self, x, register_hook=False):
        # register_hook 传入梯度hook，中间变量梯度保存
...
# Vit
class VisionTransformer(nn.Module):#实现了整个 Vision Transformer 模型，包括图像的 patch embedding、位置编码、多个块组成的层以及最后的分类器。
    """ Vision Transformer
...
# pos_embed_checkpoint 插入 visual_encoder（Vit）中
def interpolate_pos_embed(pos_embed_checkpoint, visual_encoder):#用于对位置编码进行插值，以适应不同尺寸的输入图像
...
def createVisualModel(resolution_after=224):# 用于创建预训练的 Visual Transformer 模型，并加载预训练参数。
