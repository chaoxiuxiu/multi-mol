...
class MoleculeModel(nn.Module):
    ...
    def __init__(self, classification: bool, multiclass: bool, pretrain: bool):
        ...
        if self.classification:
            self.sigmoid = nn.Sigmoid()  ##分类模型使用 Sigmoid
        self.multiclass = multiclass
        if self.multiclass:
            self.multiclass_softmax = nn.Softmax(dim=2)  ##多类别分类使用 Softmax。
    ...
    def create_ffn(self, args: Namespace):
        ...
        # Create FFN layers根据 args.ffn_num_layers 配置前馈网络层数：
        if args.ffn_num_layers == 1:  # 如果层数为 1，直接连接输入和输出层。
            ffn = [
                dropout,
                nn.Linear(first_linear_dim, args.output_size)
            ]
        else:  # 否则，添加多个隐藏层和激活函数。
            ffn = [
                dropout,
