...
def get_data(path: str,
    ...
    if args is not None:
        ...
        features_path = features_path if features_path is not None else args.features_path  # 一个字符串列表，包含特征文件的路径。
        max_data_size = max_data_size if max_data_size is not None else args.max_data_size  # 要加载的数据点的最大数量。
    ...
    return data
def get_data_from_smiles(smiles_and_labels: List[Tuple[str, List[float]]],
                         skip_invalid_smiles: bool = True,
                         args: Namespace = None,
                         features_path: List[str] = None,
                         ...
                         logger: Logger = None) -> MoleculeDataset:
    """
    Converts SMILES and labels to a MoleculeDataset, similar to get_data function.

    :param smiles_and_labels: A list of tuples, where each tuple contains a SMILES string and a list of labels.
    :param skip_invalid_smiles: Whether to skip and filter out invalid smiles.
    :param args: Arguments.
    :param features_path: A list of paths to files containing features.
    :param use_compound_names: Whether to use compound names in addition to smiles strings.
    :param logger: Logger.
    :return: A MoleculeDataset with all of the provided SMILES and labels.
    """
    debug = logger.debug if logger is not None else print
    ...
    data = MoleculeDataset([
        MoleculeDatapoint(
            ...
            features=features_data[i] if features_data is not None else None,
            use_compound_names=use_compound_names
        ) for i, (smiles, labels) in tqdm(enumerate(smiles_and_labels), total=len(smiles_and_labels))
    ])
    ...
    # Set features_dim if features are provided
    if features_data is not None and len(data) > 0:
        args.features_dim = len(data.data[0].features)
...
def get_data_from_smiles(smiles: List[str], skip_invalid_smiles: bool = True, logger: Logger = None) -> MoleculeDataset:
    ...
    return data  # MoleculeDataset 对象。
